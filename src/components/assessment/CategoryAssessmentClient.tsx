...
export default function CategoryAssessmentClient({ config, judge }: Props) {
  ...
  const [existingEvaluations, setExistingEvaluations] = useState<Record<string, any>>({});
      ...
      try {
        ...
        data.rows?.forEach((row: any) => {
          ...
          if (row.category === config.key) {
            console.log("Row matches category, adding to evaluations");
            completed.add(row.studentId);
            // เก็บคะแนนล่าสุดของนักศึกษาแต่ละคน
            scores[row.studentId] = row.totalScore;
            // เก็บข้อมูลการประเมินไว้สำหรับแก้ไข
            const key = `${row.studentId}_${row.judgeId}`;
            evaluations[key] = row;
            console.log("Added evaluation with key:", key);
          }
        });
  ...
  const getExistingEvaluation = useCallback(() => {
    if (!selectedStudent || !judge) return null;
    const key = `${selectedStudent.id}_${judge.id}`;
    const existing = existingEvaluations[key] || null;
    console.log("=== Debug getExistingEvaluation ===");
    console.log("Selected student:", selectedStudent?.id);
    console.log("Judge:", judge?.id);
    console.log("Key:", key);
    console.log("Existing evaluation:", existing);
    console.log("All evaluations:", existingEvaluations);
    return existing;
  }, [selectedStudent, judge, existingEvaluations]);

  // โหลดคะแนนเดิมเมื่อมีการประเมินอยู่แล้ว
  useEffect(() => {
    const existing = getExistingEvaluation();
    console.log("=== Debug useEffect loading scores ===");
    console.log("Existing evaluation:", existing);
    console.log("Existing scores:", existing?.scores);
    
    if (existing && existing.scores) {
      console.log("Setting scores from existing evaluation:", existing.scores);
      setScores(existing.scores);
    } else {
      console.log("No existing evaluation found, setting empty scores");
      setScores({});
    }
  }, [selectedStudent, judge, getExistingEvaluation]);
    ...
    try {
      ...
      const res = await fetch("/api/submit", {
        ...
        body: JSON.stringify({
          ...
          maxScore,
          isEdit, // บอกว่าเป็นการแก้ไขหรือไม่
        }),
      });
      ...
      setSubmitMessage(isEdit ? "อัปเดตคะแนนสำเร็จแล้ว" : "บันทึกข้อมูลสำเร็จแล้ว");
      
      // อัปเดตข้อมูลใน local state
      if (selectedStudent) {
        setCompletedSubmissions(prev => new Set([...prev, selectedStudent.id]));
        setStudentScores(prev => ({ ...prev, [selectedStudent.id]: grandTotal }));
        
        // อัปเดต existing evaluations
        const key = `${selectedStudent.id}_${judge.id}`;
          ...
          <div className="space-y-8 md:space-y-10">
            <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
              ...
              <div className="bg-white rounded-2xl px-4 py-3 shadow-sm">
                ...
                <div className="text-xs text-[#191c1d]/55">{judge.dept}</div>
              </div>
            </div>
                    ...
                    >
                      <div className="space-y-4 relative z-10">
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <div className="text-[10px] font-bold uppercase opacity-40 tracking-widest">คะแนนที่ได้รับ</div>
                            <div className="text-sm font-bold text-green-600">{studentScores[s.id]} คะแนน</div>
                          </div>
                          <button
                            onClick={() => {
                            ...
                            className="ml-auto flex items-center gap-2 px-3 py-1.5 bg-blue-500 text-white text-xs font-medium rounded-full hover:bg-blue-600 transition-colors"
                          >
                            <Edit size={12} />
                            แก้ไขคะแนน
                          </button>
                        ...
                        <div className="grid md:grid-cols-1 gap-5">
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="text-sm italic opacity-80 leading-relaxed mt-1">{s.project}</div>
                            </div>
                          </div>
                        </div>
                      </div>
          ...
          <div className="space-y-8 md:space-y-10">
            <header className="text-center space-y-6">
              ...
              {(() => {
                ...
                return (
                  <>
                    <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold tracking-tight bg-gradient-to-r from-[#191c1d] to-[#5f00e3] bg-clip-text text-transparent max-w-4xl mx-auto leading-tight">
                      {summary}
                    </h1>
                    {description && (
              ...
              <div className="flex items-center justify-center gap-6">
                ...
                <div className="flex items-center gap-3 px-4 py-2 bg-white rounded-full shadow-lg border border-gray-200">
                  ...
                  <div className="text-left">
                    ...
                    <div className="font-bold text-sm">{judge?.name}</div>
                    <div className="text-[10px] opacity-60">{judge?.dept}</div>
                  </div>
                </div>
              </div>
            </header>
          ...
          <div className="space-y-8 md:space-y-10">
            ...
            <div className="grid md:grid-cols-[1fr_340px] gap-8 items-start">
              ...
              <div className="md:sticky md:top-24 space-y-6">
                  ...
                  <div className="relative z-10 space-y-6">
                    ...
                    <div className="space-y-3 pt-6 border-t border-white/10">
                      <div className="flex justify-between items-start text-xs gap-2">
                        ...
                        <span className="font-bold text-right">{judge?.name}</span>
                      </div>
                    </div>
                  </div>
                ...
                >
                  {isSubmitting ? "กำลังบันทึก..." : (getExistingEvaluation() ? "อัปเดตคะแนน" : "ส่งผลคะแนนอย่างเป็นทางการ")}
