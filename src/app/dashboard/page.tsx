...
import { RefreshCw, Trophy, Users, ClipboardList, TrendingUp } from "lucide-react";
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <div className="text-xs text-gray-500">
                <div className="font-medium">หลักสูตร / สำนักวิชา</div>
                <div>{student.program} · {student.school}</div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-500">{student.workplace}</p>
        ...
        <div className="flex flex-col items-end gap-2 flex-shrink-0">
          <div className="text-right">
            ...
            <div className="flex items-baseline gap-1">
              <span className="text-3xl font-bold tabular-nums text-[#9f4200]">{student.avgPct.toFixed(2)}</span>
              <span className="text-xs text-gray-500">/{student.maxScore}</span>
            </div>
          </div>
        </div>
      </div>
...
export default function DashboardPage() {
  ...
  // Count judges for selected category from judges.json
  const categoryJudges = useMemo(() => {
    if (activeCategory === "all") {
      return judges.length;
    }
    
    // Count judges who have permission to evaluate this category
    return judges.filter(judge => 
      judge.categories && judge.categories.includes(activeCategory)
    ).length;
  }, [judges, activeCategory]);
        ...
        <section className="bg-white rounded-2xl px-6 py-6 shadow-[0_8px_24px_rgba(25,28,29,0.06)]">
          <div className="flex items-center justify-between gap-4 mb-6 flex-wrap">
            <div>
              ...
              <p className="text-sm text-[#191c1d]/60">กรรมการทั้งหมด {categoryJudges} คน</p>
            </div>
                  ...
                  {data.rows.map((r, i) => (
                    <tr key={i} className="border-b border-[#f8f8f8] last:border-0 hover:bg-[#fafafa]">
                      <td className="py-1.5 pr-3 text-[#191c1d]/40 whitespace-nowrap">
                        <div>{new Date(r.timestamp).toLocaleString("th-TH", { dateStyle: "short" })}</div>
                        <div>{new Date(r.timestamp).toLocaleString("th-TH", { timeStyle: "short" })}</div>
                      </td>
