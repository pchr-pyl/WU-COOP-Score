    ...
    <main className="relative min-h-screen bg-[#f8f9fa] text-[#191c1d] px-4 py-10 sm:px-5 sm:py-12 md:px-8 md:py-20 leading-[1.75] overflow-hidden">
      ...
      <div className="relative mx-auto max-w-6xl space-y-8 md:space-y-10">
        <header className="bg-white/80 backdrop-blur-xl rounded-[1.5rem] md:rounded-[1.75rem] px-4 py-6 sm:px-6 sm:py-7 md:px-10 md:py-10 shadow-[0_20px_40px_rgba(25,28,29,0.04)]">
          <div className="grid md:grid-cols-[160px_1fr] gap-5 sm:gap-7 md:gap-10 items-center">
            ...
            <div className="space-y-2 sm:space-y-3 text-center md:text-left">
              ...
              <h1 className="text-2xl sm:text-3xl md:text-5xl font-bold tracking-tight">ระบุตัวตนกรรมการผู้ประเมิน</h1>
        ...
        <section className="grid gap-6 lg:grid-cols-[420px_1fr] items-start">
          <div className="bg-white rounded-[1.5rem] p-6 md:p-8 shadow-[0_20px_40px_rgba(25,28,29,0.04)] space-y-5">
          ...
          <div className="space-y-5">
            <div className="bg-white rounded-[1.5rem] p-6 md:p-8 shadow-[0_20px_40px_rgba(25,28,29,0.04)]">
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div>
                  <p className="text-xs uppercase tracking-[0.22em] text-[#5f00e3]/60 font-semibold">Allowed Categories</p>
                  <h2 className="text-2xl font-bold tracking-tight">ประเภทที่กรรมการมีสิทธิ์ประเมิน</h2>
                </div>
              </div>

              {!activeJudge ? (
                <div className="mt-5 rounded-[1.25rem] border border-dashed border-[#191c1d]/12 px-5 py-8 text-sm text-[#191c1d]/50">เมื่อยืนยันตัวตนแล้ว ระบบจะแสดงเฉพาะประเภทที่อาจารย์ท่านนี้มีสิทธิ์ประเมิน</div>
              ) : (
                <div className="mt-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                  {allowedCategories.map((category) => (
                    <button
                      ...
                      className="text-left rounded-[1.25rem] bg-[#f3f4f5] px-5 py-5 hover:bg-gradient-to-br hover:from-[#9f4200] hover:to-[#fe6c00] hover:text-white transition-all duration-300"
                    >
                      <p className="text-[11px] uppercase tracking-[0.22em] opacity-50">Path {CATEGORY_META[category].href}</p>
                      <p className="text-xl font-bold mt-3 leading-tight">{CATEGORY_META[category].title}</p>
