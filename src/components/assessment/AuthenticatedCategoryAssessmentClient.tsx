...
export default function AuthenticatedCategoryAssessmentClient({ config }: Props) {
  ...
  // คำนวณจำนวนกรรมการที่มีสิทธิ์ในหมวดนี้
  const judgeCount = useMemo(() =>
    judges.filter(j => j.categories.includes(config.key)).length
  , [judges, config.key]);

  // ดึงข้อมูลคะแนนที่ประเมินแล้วจาก /api/dashboard
  useEffect(() => {
    let alive = true;
    const run = async () => {
      try {
        ...
        data.rows?.forEach((row: any) => {
          if (row.category === config.key) {
            ...
            evaluations[`${row.studentId}_${row.judgeId}`] = row;
            // เก็บนักศึกษาที่ประเมินแล้ว
  ...
  const getExistingEvaluation = useCallback(() => {
    if (!selectedStudent || !activeJudge) return null;
    return existingEvaluations[`${selectedStudent.id}_${activeJudge.id}`] || null;
  }, [selectedStudent, activeJudge, existingEvaluations]);
  ...
  }, [selectedStudent, activeJudge, getExistingEvaluation]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (selectedStudent) {
        ...
        const key = `${selectedStudent.id}_${activeJudge.id}`;
          ...
          <div className="space-y-8 md:space-y-10">
            <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
              <header className="space-y-3">
                ...
                <div className="mt-2 text-green-700 text-sm font-bold flex items-center gap-2">
                  จำนวนกรรมการที่มีสิทธิ์ให้คะแนน <span className="text-2xl font-extrabold text-green-700">{judgeCount}</span> คน
                </div>
            ...
            {loadingStudents ? (
              ...
              <div className="space-y-6">
                {students.map((student) => (
                  completedSubmissions.has(student.id) ? (
                    <div
                      key={student.id}
                      ...
                      <div className="space-y-4 relative z-10">
                        <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4">
                          ...
                          <div className="flex items-center gap-2">
                            <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-gray-600">{student.name}</h2>
                        ...
                        <div className="grid md:grid-cols-1 gap-5">
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              ...
                              <div className="text-sm font-medium text-gray-600">{student.program} · {student.school}</div>
                            </div>
                          </div>
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="text-sm font-medium text-gray-600">{student.workplace}</div>
                              <div className="text-sm italic opacity-60 leading-relaxed mt-1 text-gray-500">{student.project}</div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-sm font-bold text-green-600">{studentScores[student.id]} คะแนน</div>
                          </div>
                        </div>
                      </div>
                    </div>
                        ...
                        <div className="grid md:grid-cols-1 gap-5">
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="text-sm italic opacity-80 leading-relaxed mt-1">{student.project}</div>
                            </div>
                          </div>
                        </div>
                      </div>
          ...
          <div className="space-y-8 md:space-y-10">
            <header className="flex flex-col md:flex-row md:items-start justify-between gap-6">
              <div className="space-y-2">
                ...
                <h1 className="text-xl sm:text-2xl md:text-3xl font-bold max-w-3xl leading-tight">{config.topics[step - topicStartStep].title}</h1>
              </div>
