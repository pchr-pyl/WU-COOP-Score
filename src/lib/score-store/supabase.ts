...
export async function checkExistingSubmission(studentId: string, judgeId: string, category: string) {
  const response = await fetch(
    buildRestUrl(
      "/score_submissions",
      `select=*&student_id=eq.${studentId}&judge_id=eq.${judgeId}&category=eq.${category}&limit=1`
    ),
    {
      method: "GET",
      headers: {
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(text || "Unable to check existing submission");
  }
...
export async function updateSubmission(studentId: string, judgeId: string, category: string, payload: SubmitPayload) {
  const response = await fetch(
    buildRestUrl(
      "/score_submissions",
      `student_id=eq.${studentId}&judge_id=eq.${judgeId}&category=eq.${category}`
    ),
    {
      method: "PATCH",
      headers: {
